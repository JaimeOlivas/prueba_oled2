...
void i2c_master_init(SSD1306_t * dev, int16_t sda, int16_t scl, int16_t reset)
{
	...
	if (reset >= 0) {
		...
		vTaskDelay(50 /  ( TickType_t ) 1000 / 100);
...
void i2c_init(SSD1306_t * dev, int width, int height) {
	...
	esp_err_t espRc = i2c_master_cmd_begin(I2C_NUM, cmd, 10/( TickType_t ) 1000 / 100);
	if (espRc == ESP_OK) {
...
void i2c_display_image(SSD1306_t * dev, int page, int seg, uint8_t * images, int width) {
	...
	i2c_master_stop(cmd);
	i2c_master_cmd_begin(I2C_NUM, cmd, 10/( TickType_t ) 1000 / 100);
	i2c_cmd_link_delete(cmd);
...
void i2c_hardware_scroll(SSD1306_t * dev, ssd1306_scroll_type_t scroll) {
	...
	i2c_master_stop(cmd);
	espRc = i2c_master_cmd_begin(I2C_NUM, cmd, 10/ ( TickType_t ) 1000 / 100);
	if (espRc == ESP_OK) {
